      ...
      class FiltersBase
        ...
        def get_properties_scope
          ...
          @properties[:property_ids].each do |id|
            inter_strings<<Spree::Product.select('spree_products.id').joins(:product_properties).search(:product_properties_property_id_eq => id.first).result.where("#{id.second.values.map{|e| "lower(spree_product_properties.value) LIKE lower('#{e}')"}.join(' OR ')}").to_sql
          end
        ...
        def add_sort_scope(base_scope)
          if @properties[:sort].present?
            ...
            base_scope = base_scope.reorder('').joins(:classifications).where("spree_products_taxons.taxon_id"=>@properties[:taxon].id).order("spree_products_taxons.position") if @properties[:taxon]
